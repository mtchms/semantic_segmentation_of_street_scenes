...
- \(1 \times 1\) свертка для уменьшения размерности (сжатие)
- \(3 \times 3\) свертка для извлечения пространственных признаков
- \(1 \times 1\) свертка для восстановления размерности (расширение в 4 раза)
...
**Структура энкодера:**
- Начальная свертка \(7 \times 7\) со stride=2
...
**Компоненты ASPP:**
- **Branch 1**: \(1 \times 1\) свертка (базовые признаки)
- **Branch 2**: \(3 \times 3\) dilated свертка с dilation=6
- **Branch 3**: \(3 \times 3\) dilated свертка с dilation=12
- **Branch 4**: \(3 \times 3\) dilated свертка с dilation=18
- **Global pooling**: адаптивный average pooling + \(1 \times 1\) свертка

Все ветви выдают 256 каналов. Выходы конкатенируются (1280 каналов) и проецируются обратно в 256 каналов через \(1 \times 1\) свертку с dropout (p=0.5).
...
\[
\alpha = \sigma(W_{\psi}(\text{ReLU}(W_g \cdot g + W_x \cdot x)))
\]
\[
\hat{x} = x \odot \alpha
\]
...
1. ASPP выход upsampling до размера \(H/4 \times W/4\) (bilinear interpolation)
2. Skip connection (f1) проходит через attention gate (опционально)
3. Skip connection редуцируется до 48 каналов через \(1 \times 1\) свертку
4. Конкатенация ASPP (256) и skip (48) → 304 канала
5. Два последовательных \(3 \times 3\) свертки с 256 каналами
6. Upsampling до оригинального разрешения \(H \times W\)
7. Финальная \(1 \times 1\) свертка для классификации на num_classes
...
- **layers**: конфигурация ResNet [3,4,6,3] для ResNet-50
